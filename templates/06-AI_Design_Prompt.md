...
- Backend storage mapping in `examsService.buildExamPackageDoc` enforces the rule.
- Builder API: `TheoreticalQuestionBuilder(allowExternalDifficulty=false)` controls external honoring.

Traceability:

- Linked to feature: Exam Questions → Difficulty Normalization.
- Referenced in architecture flow and feature specs.
